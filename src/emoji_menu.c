...
static void graphics_layer_update_callback(Layer *layer, GContext *ctx) {
  ...
  if (page_selected==PAGE_CHILD) {  
    ...
    uint8_t h = EMOJI_HEIGHT-1;
    uint8_t ox = (bounds.size.w - w*EMOJI_CHILD_COLS)/2;
    uint8_t oy = (bounds.size.h - h*EMOJI_CHILD_ROWS)/2;
    uint8_t i=0;
    while (*stickers) {
    ...
    for (uint8_t y=0; y<EMOJI_PAGE_ROWS; y++) {
      for (uint8_t x=0; x<EMOJI_PAGE_COLS; x++) {
        graphics_draw_bitmap_in_rect(ctx, main_get_emoji(tab_selected,x,y,0), GRect(round_xy[i][0], round_xy[i][1], EMOJI_WIDTH, EMOJI_HEIGHT));
        i++;
      }
    }
    ...
    if (page_selected==PAGE_EMOJIS) {
      ...
      graphics_draw_round_rect(ctx, GRect(round_xy[emoji_selected][0]-2, round_xy[emoji_selected][1]-1, EMOJI_WIDTH+4, EMOJI_HEIGHT+2),3); // outer square
      graphics_draw_round_rect(ctx, GRect(round_xy[emoji_selected][0]-1, round_xy[emoji_selected][1]  , EMOJI_WIDTH+2, EMOJI_HEIGHT  ),2); // inner square
    }  
  } 
}

#else //PBL_ROUND  - the following is for rectangular
...
static void graphics_layer_update_callback(Layer *layer, GContext *ctx) {
  ...
  // tabs
  uint8_t th = bounds.size.h / MAX_EMOJI_PAGES;
  uint8_t tox = bounds.size.w - EMOJI_WIDTH;
  uint8_t toy = 2+(th - EMOJI_HEIGHT)/2; 
  // grid 
  uint8_t w = bounds.size.w / (EMOJI_PAGE_COLS+1);
  uint8_t h = bounds.size.h / EMOJI_PAGE_ROWS;
  uint8_t ox = 2+(w - EMOJI_WIDTH)/2;
  uint8_t oy = toy + 1 + h + (h - EMOJI_HEIGHT)/2;
  ...
  if (page_selected==PAGE_CHILD) {  
    ...
    if (page_selected==PAGE_EMOJIS) {
      // highlight selected emoji
      graphics_context_set_stroke_color(ctx,GColorBlack);
      graphics_draw_round_rect(ctx, GRect(ox+w*(emoji_selected % EMOJI_PAGE_COLS)-2, oy+h*(emoji_selected / EMOJI_PAGE_COLS)-1, EMOJI_WIDTH+4, EMOJI_HEIGHT+2),3); // outer square
      graphics_draw_round_rect(ctx, GRect(ox+w*(emoji_selected % EMOJI_PAGE_COLS)-1, oy+h*(emoji_selected / EMOJI_PAGE_COLS)  , EMOJI_WIDTH+2, EMOJI_HEIGHT  ),2); // inner square
    }  
  }
}

#endif // else PBL_ROUND
...
static void initialise_ui(void) {
  ...
  uint8_t w=EMOJI_WIDTH -1;
  uint8_t ox=EMOJI_WIDTH-5;
  for (uint8_t y=0; y<EMOJI_PAGE_ROWS; y++) {
    for (uint8_t x=0; x<EMOJI_PAGE_COLS; x++) {
      if ( x==0 && (y==0 || y==EMOJI_PAGE_ROWS-1) ) continue;
      h=EMOJI_HEIGHT+(x==2?1:-2); // give middle row more vertical space
